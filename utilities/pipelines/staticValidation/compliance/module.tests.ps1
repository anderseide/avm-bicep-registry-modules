...
Describe 'File/folder tests' -Tag 'Modules' {
    ...
    Context 'General module folder tests' {
        ...
        foreach ($moduleFolderPath in $moduleFolderPaths) {
            $null, $moduleType, $resourceTypeIdentifier = ($moduleFolderPath -split '[\/|\\]\w+[\/|\\](res|ptn|utl)[\/|\\]') # 'avm|pavm/res|ptn|utl/<provider>/<resourceType>' would return 'modules', 'res|ptn|utl', '<provider>/<resourceType>'
        ...
        It '[<moduleFolderName>] Module should contain a [` ORPHANED.md `] file only if orphaned.' -TestCases ($moduleFolderTestCases | Where-Object { $_.isTopLevelModule }) {
            ...
            if ($moduleFolderPath -notmatch '^avm/') {
                Set-ItResult -Skipped -Because 'Private modules are not expected to have an ORPHANED.md file.'
                return
            }
    ...
    Context 'Top level module folder tests' {
        ...
        <<<<<<< HEAD:avm/utilities/pipelines/staticValidation/compliance/module.tests.ps1
                    ...
                    BeforeDiscovery {
                        ...
                        foreach ($moduleFolderPath in $moduleFolderPaths) {
                            ...
                            if (($resourceTypeIdentifier -split '[\/|\\]').Count -eq 2) {
                                ...
                                >>>>>>> upstream/main:utilities/pipelines/staticValidation/compliance/module.tests.ps1
                            }
                        }
                    }
                ...
                ($Schemaverion.Substring(0, 5) -eq 'https') | Should -Be $true
                        ...
                        Context 'Resources' {
                            ...
                            It '[<moduleFolderName>] Telemetry deployment should have expected telemetry identifier.' -TestCases ($moduleFolderTestCases | Where-Object { $_.isTopLevelModule }) {
                                ...
                                if ($templateFilePath -notmatch '^\./avm/') {
                                    Set-ItResult -Skipped -Because 'Private modules are not expected to have an telemetry identifier.'
                                    return
                                }
