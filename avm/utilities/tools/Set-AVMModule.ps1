...
function Set-AVMModule {
    ...
    if (-not $SkipFileAndFolderSetup -and (($resolvedPath -split '[\\|\/]\w+[\\|\/](res|ptn|utl)[\\|\/].+?[\\|\/].+').count -gt 1)) {
    ...
    if ($PSCmdlet.ShouldProcess(('Building & generation of [{0}] modules in path [{1}]' -f $relevantTemplatePaths.Count, $resolvedPath), 'Execute')) {
        try {
            $job = $relevantTemplatePaths | ForEach-Object -ThrottleLimit $ThrottleLimit -AsJob -Parallel {
                $identifierElements = $_ -split '[\/|\\](\w+)[\/|\\](res|ptn|utl)[\/|\\]'
                $resourceTypeIdentifier = ('{0}/{1}/{2}' -f $identifierElements[1], $identifierElements[2], $identifierElements[3]) -replace '\\', '/' # modules/res/<provider>/<resourceType>
